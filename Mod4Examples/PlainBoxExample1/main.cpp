/** @file main.cpp
  ...
  *
  * No virtual methods are defined
  *
...
{  cout << "Test to see that MagicBox contents can only be set once." << endl;
   ...
   test2();		
	...
	cout << "Compiler knows that the first parameter of the placeInBox function is a PlainBox.  Therefore, " << endl
	     << "  the setItem method of the PlainBox class is called which allows the item to be changed. " << endl;
   ...
   test4();
	...
	cout << "Since the array contains both PlainBox and MagicBox objects, the array must be of the super class type.  Hence, " << endl
	     << "  the array is a PlainBox.  The compiler associates all calls to the setItem method of the PlainBox class because " << endl
	     << "  the array is of PlainBox type.  Therefore, all items can be changed. " << endl;
