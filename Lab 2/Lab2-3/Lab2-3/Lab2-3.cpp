...
int isMember(int myArray[], int search, int start, int numElems){
    //cout<<"isMember RUN: "<<search<<" "<<start<<" "<<numElems;
    //cout<<"Current array entry: "<<myArray[start];
    //if hit
    int index = -5;
    if(start == numElems)
        index = -1;
        ...
        if(myArray[start] == search){
            //cout<<"inside if statement for hit";
            index = start;
            ...
            //cout<<"the value of index is: "<<endl;
        }
    }
    //cout<<"SECOND INDEX COUT: "<<index;
    return(index);
}